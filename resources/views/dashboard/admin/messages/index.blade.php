                ...
                <div class="mt-4 space-y-2">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="filterMessages('all')"
                            class="filter-btn active px-3 py-1 text-sm rounded-full bg-blue-100 text-blue-600">
                            <i class="fas fa-layer-group mr-1"></i> All
                        </button>
                        <button onclick="filterMessages('unread')"
                            class="filter-btn px-3 py-1 text-sm rounded-full border border-gray-300 text-gray-600 hover:bg-gray-50">
            ...
            <div class="flex-1 overflow-y-auto" id="messagesList">
                @forelse($messages as $message)
                    <div class="message-item border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors duration-150 {{ $message->read ? 'bg-white' : 'bg-blue-50' }}"
                        data-message-id="{{ $message->id }}" data-read="{{ $message->read ? 'true' : 'false' }}">
                        <div class="p-4">
...
@push('scripts')
    <script>
        ...
        async function markAsRead(messageId) {
            const markMessageReadUrl = "{{ route('admin.mark.messages.read', ':id') }}";
            let url = markMessageReadUrl.replace(':id', messageId);
        ...
        async function markAsUnread(messageId) {
            const markMessageUnReadUrl = "{{ route('admin.mark.messages.unread', ':id') }}";
            let url = markMessageUnReadUrl.replace(':id', messageId)
            try {
                ...
                if (response.ok) {
                    ...
                    if (messageItem) {
                        ...
                        if (nameSpan && !messageItem.querySelector('.bg-blue-100')) {
                            ...
                            badge.className =
                                'inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 ml-2';
